...
class Ui_LoginFaceWindow(object):
    ...
    def View_video(self):
        ...
        if self.count < 5:
            self.count +=1
        if self.count == 5:
            # QtCore.QThread(target=self.RecognizeFace,ar).start()
    ...
    def RecognizeFace(self):
        ...
        if len(self.bboxes)> 0:
            ...
            if  maxvalue > 0.3:
                ...
                self.CheckNameLabel.setText("{}".format(self.Name))
                self.CheckIDLabel.setText("{}".format(self.ID))
                bboxes, kpss = self.detector.detect(self.img , input_size =(640, 640))
        
                xmin,ymin,xmax,ymax,score = [int(_) for _ in bboxes[0]][:5]
                FaceImage = self.img [ymin:ymax,xmin:xmax]
                
                self.LoadImage(FaceImage)
                self.StateNext = True
                self.StaffName = self.UserCollection.find_one({"_id":ObjectId(self.ID)})["Name"]
                self.timer.stop()
                self.cap.release()
