...
"}")
    ...
    def Delete(self):
        ...
        if idx >= 0:
            ...
            x = self.HistoryCollection.find_one({'_id': ObjectId(ID)})
            self.HistoryCollection.delete_one(x)
    ...
    def loaddata(self):
        ...
        Dataset = []
        for HistoryDocument in self.HistoryCollection.find():
            Dataset.append(HistoryDocument)
        ...
        for Data in Dataset:
            ...
            timein = time.ctime(Data["TimeIN"])
            self.tableWidget.setItem(row, 2, QtWidgets.QTableWidgetItem(timein))
            timeout = time.ctime(Data["TimeOUT"])
            self.tableWidget.setItem(row, 3, QtWidgets.QTableWidgetItem(timeout))
            self.tableWidget.setItem(row, 4, QtWidgets.QTableWidgetItem(str(Data["TotalTime"])))
            # self.tableWidget.setItem(row, 5, QtWidgets.QTableWidgetItem(str(Data["Cost"])+" VND"))
            self.tableWidget.setItem(row, 5, QtWidgets.QTableWidgetItem(Data["StaffName"]))
            row = row + 1
